...
if tokens > 0 then
  tokens = tokens - 1
  allowed = 1
  redis.log(redis.LOG_NOTICE, "[TokenBucket] Request allowed. tokens left=" .. tostring(tokens))
else
  redis.log(redis.LOG_NOTICE, "[TokenBucket] Request throttled. tokens left=" .. tostring(tokens))
end
...
redis.log(redis.LOG_NOTICE, "[TokenBucket] State updated: key=" .. tostring(key) .. ", tokens=" .. tostring(tokens) .. ", last_refill=" .. tostring(now))
